...
```
**From the plot we can tell that when only 4 parameters are included, the RMES is minimized, so 4 predictors are included in the model when 1SE rule is applied. The test error of this model is `r test_error_lasso`**
...
```
**The selected best parameter is alpha = 0.1 and lambda = 7.39. The test error is `r test_error_enet`.**
...
```
**14 components are included in the model. The test error of this model is `r test_error_pls`.**
